...
power_result_t power_init(void)
{
    ...
    printf("Power monitoring initialized - Battery: %.2fV (%d%%)\n",
           current_power_status.battery_voltage, current_power_status.battery_percent);
...
static void power_update_status(void)
{
    ...
    current_power_status.battery_percent =
        power_voltage_to_percent(current_power_status.battery_voltage);
