...
// Menu definitions
static const menu_item_t main_menu[] = {{"Scan JTAG", MENU_JTAG_SCAN, action_jtag_scan},
...
static const menu_item_t jtag_menu[] = {{"Auto Scan", MENU_MAIN, action_jtag_scan},
...
static const menu_item_t logs_menu[] = {{"View Logs", MENU_MAIN, action_view_logs},
        ...
        if (scan_result.device_count > 0) {
            ...
            for (uint8_t i = 0; i < scan_result.device_count; i++) {
                printf("Device %d: %s %s (IDCODE: 0x%08lX)\n", i,
                       scan_result.devices[i].manufacturer, scan_result.devices[i].device_name,
...
void action_jtag_detect_pins(void)
{
    ...
    if (jtag_detect_pins(detected_pins)) {
        ...
        printf("Detected JTAG pins - TCK:%d TMS:%d TDI:%d TDO:%d\n", detected_pins[0],
               detected_pins[1], detected_pins[2], detected_pins[3]);
    } else {
...
void action_memory_dump(void)
{
    ...
    if (bytes_read > 0) {
        char status_msg[48];
        snprintf(status_msg, sizeof(status_msg), "Dumped %lu bytes from 0x%08lX", bytes_read,
                 start_addr);
        ui_show_status(status_msg, 4000);
