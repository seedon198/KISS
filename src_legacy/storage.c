...
{
    if (!storage_mounted || !level || !message)
        return false;
    ...
    snprintf(line_buffer, sizeof(line_buffer), "[%02lu:%02lu:%02lu.%03lu] [%s] %s\n", hours,
             minutes, seconds, milliseconds, level, message);
...
{
    if (!storage_mounted || !filename || !data || size == 0)
        return false;
...
{
    if (!storage_mounted || !key || !value || max_len == 0)
        return false;
...
{
    if (!storage_mounted || !path || !listing)
        return false;
    ...
    while ((result = f_readdir(&dir, &file_info)) == FR_OK && file_info.fname[0] != 0) {
        ...
        if (listing->count >= listing->capacity) {
            listing->capacity *= 2;
            file_info_t *new_files =
                (file_info_t *)realloc(listing->files, listing->capacity * sizeof(file_info_t));
            if (!new_files) {
                break;  // Out of memory
            }
        ...
        // Convert FatFS date/time to our structures
        file->date.year   = (file_info.fdate >> 9) + 1980;
        file->date.month  = (file_info.fdate >> 5) & 0x0F;
        file->date.day    = file_info.fdate & 0x1F;
        
        file->time.hour   = file_info.ftime >> 11;
        file->time.minute = (file_info.ftime >> 5) & 0x3F;
        file->time.second = (file_info.ftime & 0x1F) * 2;
...
{
    if (!storage_mounted || !path || !buffer || max_size == 0)
        return 0;
...
{
    if (!storage_mounted || !path || !data || size == 0)
        return false;
