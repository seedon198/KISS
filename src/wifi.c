/**
 * @file wifi.c
 * @brief Wi-Fi web server implementation for KISS Fuzzer
 * 
...
static const char html_header[] = 
    ...
    "h1{color:#9c27b0;}button{background:#9c27b0;color:#fff;border:none;padding:10px 20px;margin:5px;border-radius:5px;cursor:pointer;}"
    "button:hover{background:#7b1fa2;}input,select{padding:8px;margin:5px;border-radius:3px;border:1px solid #555;background:#333;color:#fff;}"
    ".status{background:#333;padding:10px;border-radius:5px;margin:10px 0;}.log{background:#222;padding:10px;border-radius:5px;margin:10px 0;font-family:monospace;white-space:pre-wrap;max-height:300px;overflow-y:auto;}"
    "</style></head><body>";
...
static void handle_api_request(const char *path, const char *query, char *response, size_t response_size);
    ...
    if (strncmp(path, "jtag/", 5) == 0) {
        ...
        if (strcmp(action, "scan") == 0) {
            ...
            snprintf(response, response_size,
                "{\"status\":\"%s\",\"devices\":%d,\"length\":%d}",
                (result == JTAG_OK) ? "success" : "error",
                chain.device_count, chain.total_length);
