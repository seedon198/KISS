...
bool jtag_init(void)
{
    ...
    jtag_shift_program_init(jtag_pio, jtag_shift_sm, offset_shift,
                           current_config.tck_pin, current_config.tms_pin,
                           current_config.tdi_pin, current_config.tdo_pin);
    
    jtag_clock_program_init(jtag_pio, jtag_clock_sm, offset_clock,
                           current_config.tck_pin);
    
    jtag_reset_program_init(jtag_pio, jtag_reset_sm, offset_reset,
                           current_config.tck_pin, current_config.tms_pin);
...
{
    if (!jtag_initialized || !result) return false;
        ...
        if (tdo_bit == 0) {
            ...
            if ((idcode & 1) == 1 && idcode != 0xFFFFFFFF) {
                ...
                printf("Device %d: IDCODE=0x%08lX (%s %s)\n", 
                       device_count, idcode, device->description, device->description);
...
{
    if (!jtag_initialized || !idcode) return false;
...
{
    if (!jtag_initialized || !data || length == 0) return 0;
...
{
    if (!jtag_initialized) return false;
    
    printf("Boundary scan for device %d\n", device_index);
    
    // TODO: Implement boundary scan (IEEE 1149.1)
    // This requires loading SAMPLE/PRELOAD instruction and shifting boundary scan register
...
{
    if (!jtag_initialized) return current_tap_state;
...
{
    if (!jtag_initialized || length == 0 || length > 32) return 0;
    ...
    for (size_t i = 0; i < db_size; i++) {
        ...
        if ((idcode & entry->idcode_mask) == entry->idcode_value) {
            ...
            snprintf(device->description, sizeof(device->description), 
                     "%s %s", entry->manufacturer, entry->device_name);
            return true;
        }
    }
...
jtag_result_t jtag_boundary_scan(uint8_t device_index, jtag_boundary_scan_t *scan_result) {
    ...
    result = jtag_shift_ir(&sample_instr, 
                          current_chain.devices[device_index].ir_length,
                          device_index);
    if (result != JTAG_OK) {
        return result;
    }
    ...
    // Copy boundary data
    memcpy(scan_result->cell_data, boundary_data, 
           sizeof(uint32_t) * ((boundary_length + 31) / 32));
    ...
    for (uint32_t i = 0; i < boundary_length; i += 3) {
        ...
        uint32_t pin = i / 3;
        if (pin < 32) {
            if (input) scan_result->input_pins |= (1UL << pin);
            if (output) scan_result->output_pins |= (1UL << pin);
            if (control) scan_result->control_pins |= (1UL << pin);
        }
    }
...
jtag_result_t jtag_start_memory_fuzz(const jtag_fuzz_config_t *fuzz_config) {
    ...
    // Start fuzzing task
    jtag_event_t event = {
        .type = JTAG_EVENT_START_FUZZ,
        .data.fuzz_config = *fuzz_config
    };
...
jtag_result_t jtag_stop_memory_fuzz(void) {
    ...
    LOG_INFO("Errors detected: %lu", fuzz_state.errors_detected);
...
jtag_result_t jtag_power_glitch(const jtag_glitch_config_t *glitch_config) {
    ...
    if (storage_is_available()) {
        ...
        snprintf(log_entry, sizeof(log_entry),
                "GLITCH,%lu,%lu,%.2f,%lu\n",
                time_us_32(),
                glitch_config->duration_us,
                glitch_config->voltage,
                actual_duration);
...
jtag_result_t jtag_security_analysis(jtag_security_analysis_t *analysis_result) {
    ...
    for (uint8_t i = 0; i < current_chain.device_count; i++) {
        jtag_device_info_t *device = &current_chain.devices[i];
    ...
    LOG_INFO("- Accessible devices: %d/%d", 
             analysis_result->accessible_devices, current_chain.device_count);
    LOG_INFO("- Security level: %s",
             (analysis_result->security_level == JTAG_SEC_LEVEL_HIGH) ? "High" :
             (analysis_result->security_level == JTAG_SEC_LEVEL_MEDIUM) ? "Medium" : "Low");
